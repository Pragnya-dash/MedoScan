...
async def analyze(req: AnalyzeRequest):
    # AnalyzeRequest doesn't have id/selftext/content. Build raw dict for pipeline.
    raw_input = {
        "id": None,
        "source": req.source,
        "text": req.text,
...
async def stats(hours: int = Query(168, ge=1, le=720)):
    ...
    # Previous-window deltas for KPI arrows
    cutoff_now = datetime.now(timezone.utc) - timedelta(hours=hours)
    cutoff_prev = cutoff_now - timedelta(hours=hours)
...
async def heatmap(hours: int = Query(168, ge=1, le=720)):
    """Drug × symptom intensity grid for the analytics heatmap."""
    records = await _recent_posts(hours, limit=2000)
...
async def alerts(hours: int = Query(72, ge=1, le=720)):
    ...
    # Sort: critical first, then by delta_pct, then by count
...
async def refresh_insights(hours: int = Query(168, ge=1, le=720), top_n: int = Query(10, ge=1, le=30)):
    ...
    generated = 0
    for a in items:
        # Build excerpts from posts of this group
        cutoff = (datetime.now(timezone.utc) - timedelta(hours=hours)).isoformat()
...
async def seed():
    ...
    for r in results:
        await _save_post(r)
        saved += 1
    # Kick off insights refresh in the background so dashboard call is fast next
    import asyncio
...
async def _startup():
    # auto-seed only if empty to deliver wow on first load
